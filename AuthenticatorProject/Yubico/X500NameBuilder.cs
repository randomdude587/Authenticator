...
namespace AuthenticatorProject.Yubico {
    ...
    public static class X500NameElementExtensions {
        ...
        public static byte[] GetOid(this X500NameElement nameElement) {
            switch (nameElement) {
                case X500NameElement.Country: return new byte[] { 0x55, 0x04, 0x06 };
                case X500NameElement.State: return new byte[] { 0x55, 0x04, 0x08 };
                case X500NameElement.Locality: return new byte[] { 0x55, 0x04, 0x07 };
                case X500NameElement.Organization: return new byte[] { 0x55, 0x04, 0x0A };
                case X500NameElement.CommonName: return new byte[] { 0x55, 0x04, 0x03 };
                default: throw new ArgumentException(X500NameBuilder.InvalidElementMessage);
            }
        }
        ...
        public static bool IsValidValueLength(this X500NameElement nameElement, int length) {
            switch (nameElement) {
                case X500NameElement.Country: return length == 2;
                case X500NameElement.State: return (length > 0) && (length < 32);
                case X500NameElement.Locality: return (length > 0) && (length < 32);
                case X500NameElement.Organization: return (length > 0) && (length < 64);
                case X500NameElement.CommonName: return (length > 0) && (length < 64);
                default: throw new ArgumentException(X500NameBuilder.InvalidElementMessage);
            }
        }
    }
