...
namespace AuthenticatorProject.Yubico {
    ...
    public static class CertificateOperations {
        ...
        public static void GetCertRequest(IYubiKeyDevice yubiKey, Func<KeyEntryData, bool> KeyCollectorDelegate, 
            X500DistinguishedName distinguishedName, SamplePivSlotContents slotContents) {
            ...
            AsymmetricAlgorithm dotNetPubKey = KeyConverter.GetDotNetFromPivPublicKey(slotContents.PublicKey);

            // Build a cert request object.
            // This sample code uses SHA-256 for all algorithms except ECC P-384.
            // With RSA it always uses PSS.
            switch (slotContents.Algorithm) {
                case PivAlgorithm.EccP256:
                    slotContents.CertRequest = new CertificateRequest(distinguishedName, (ECDsa) dotNetPubKey, HashAlgorithmName.SHA256);
                    break;
                case PivAlgorithm.EccP384:
                    slotContents.CertRequest = new CertificateRequest(distinguishedName, (ECDsa) dotNetPubKey, HashAlgorithmName.SHA384);
                    break;
                default:
                    slotContents.CertRequest = new CertificateRequest(distinguishedName, (RSA) dotNetPubKey, HashAlgorithmName.SHA256, RSASignaturePadding.Pss);
                    break;
            }
        ...
        public static void GetSelfSignedCert(IYubiKeyDevice yubiKey, Func<KeyEntryData, bool> KeyCollectorDelegate, SamplePivSlotContents slotContents) {
        ...
        */
        ...
        public static CertificateRequest BuildCertRequestFromDer(byte[] requestDer, HashAlgorithmName signerHash) {
            AsymmetricAlgorithm requestPublicKey = GetComponentsFromCertRequestDer(
                requestDer,
                out byte[] _,
        ...
        */
        ...
        public static bool IsValidCertRequestSignature(char[] certRequestPem) {
            ...
            AsymmetricAlgorithm pubKey = GetComponentsFromCertRequestDer(
                request,
                out byte[] toBeSigned,
        ...
        */
        ...
        public static bool IsValidCertificateSignature(
            ...
            AsymmetricAlgorithm pubKey = GetPublicKeyFromCertificate(issuerCert);
            ...
            // verification method needs.
            PivAlgorithm algorithm;
            switch (pubKey.KeySize) {
                case 256: algorithm = PivAlgorithm.EccP256; break;
                case 384: algorithm = PivAlgorithm.EccP384;break;
                default: algorithm = PivAlgorithm.None; break;
            }
            \// The signature is a BIT FIELD so the first octet is the unused
            // bits. That's why in the following we use a Slice of the signature
            // buffer.
